Comp 116 Homework 1
Matt Brenman
Due: 9/17/2013 11:59 PM

Sources referenced:
Reconstructing files: http://shankaraman.wordpress.com/tag/how-to-extract-ftp-files-from-wireshark-packet/
Info on FTP: http://engineering.deccanhosts.com/2013/02/why-is-ftp-insecure.html

set1.pcap

1. How many packets are there in this set? 276 packets

2. What protocol was used to transfer files from PC to server? FTP

3. Briefly describe why the protocol used to transfer the files is insecure? FTP sends all data (for uploading and downloading) in the clear, so nothing in encrypted. This means that anyone sniffing the network (no matter if it is switched or unswitched) can reconstruct files and also find the destination IP, username, and passwords used.

4. What is the secure alternative to the protocol used to transfer files? Instead of using FTP, users should use SFTP, which stands for SSH File Transfer Protocol or Secure File Transfer Protocol. SFTP, unlike FTP, encrypts all data (such as files and credentials). Using this system, it is necessary to break the encryption in order to view the data, so the security depends now on the principles of crypto instead of nothing.

5. What is the IP address of the server? 67.23.79.113

6. What was the username and password used to access the server? USER: stokerj PASS: wOOtfu!

7. How many files were transferred from PC to server? Three files were transferred from PC to server.

8. What are the names of the files transferred from PC to server? code.rtf, secret.pdf, acb.jpg, 

9. Extract all the files that were transferred from PC to server. These files must be part of your submission!

set2.pcap 

10. How many packets are there in your set? 74566 packets

11. How many plaintext username-password pairs are there in this packet set? I found nine plaintext username password pairs in the set.

12. Briefly describe how you found the username-password pairs. I did a string search in Wireshark for "PASS", "login", "username", "successful", "failure", "user", and then followed the TCP stream to see the conversation, which gave the user and pass

13. For each of the plaintext username-password pair that you found, identify the protocol used, server IP, the corresponding domain name (e.g., google.com), and port number.
       User: e129286 Pass: Ekkama --FAILED
               Protocol: POP
               server IP: 144.122.144.179
               domain name: imap.metu.edu.tr
               port number:  pop3 (110)
       User: mbergeson@hjnews.com Pass: mb123on --SUCCESS
               Protocol: POP
               server IP: 67.128.149.178
               domain name: mail.newswest.com
               port number:  pop3 (110)
       User: brewer Pass: 1qazxsw209simona12 --SUCCESS
               Protocol: POP
               server IP: 62.173.185.22
               domain name: www.imartini.it
               port number: pop3 (110)
       User: cisco Pass: 185 chris10 --FAILED
               Protocol: TELNET
               server IP: 200.60.17.1
               domain name: 
               port number: telnet (23)
       User: cisco Pass: 185 chelita --FAILED
               Protocol: TELNET
               server IP: 200.60.17.1
               domain name: 
               port number: telnet (23)
       User: cisco Pass: 185 chayank --FAILED
               Protocol: TELNET
               server IP: 200.60.17.1
               domain name: 
               port number: telnet (23)
       User: cisco Pass: 185 cereza --FAILED
               Protocol: TELNET
               server IP: 200.60.17.1
               domain name: 
               port number: telnet (23)
       User: cisco Pass: 185 buburuza --UNCLEAR (Assumed successful)
               Protocol: TELNET
               server IP: 200.60.17.1
               domain name: 
               port number: telnet (23)
       User: martini@cutaway.it Pass: se1lasa --SUCCESSFUL
               Protocol: IMAP
               server IP: 109.168119.166
               domain name: mail.cutaway.it
               port number: 143
...
14. Of all the plaintext username-password pairs that you found, how many of them are legitimate? That is, the username-password was valid, access successfully granted?
       Three were successful, and five failed. One is unclear, but based on similar attempts, it seems like it was successful or interrupted, which would make three definitely successful attempts.

15. How did you verified the successful username-password pairs?
       After following the TCP stream, the site would either send a confirmation of login (ex: "+OK Logged in.") or a send a failure message (ex: "Login invalid"). For one, there was no response, so I am not sure.
...
16. In a few words, explain why I asked you not to log on to websites or services associated with the username-password pairs that you found.
       We were asked not to do this for two reasons. One, they could be fraudulent websites designed to attack somebody sniffing the network who decides to visit. Two, logging onto a website with somebody else's credentials would not be legal and could get us in trouble.
...
17. What advice would you give to the owners of the username-password pairs that you found so their account information would not be revealed "in-the-clear" in the future?
       I would advise them against using forms that use protocols that are known to be unsecured, like HTTP and TELNET. When possible, they should make sure that whatever they are using uses a form of encryption, like SSL, for communication. I would also advise them not to log on to any site (or public wifi) at DefCon, since sniffing is relatively easy, and not all sites are secure. They should only use networks where they can everyone with access, although this is very difficult to do in practice.
...
Below is the list of all IP-host pairs. I used Wiresharks built-in Address Resolution tool, and I copied the results below.

# Hosts information in Wireshark 
# Host data gathered from /Users/mattbrenman/Downloads/set2.pcap
...
Here is a Protocol Hierarchy of the protocols used in set2.pcap. To get this information, I used Wireshark's built in Protocol Hierarchy tool. Note: All percentages in terms of packets, not bytes. The most popular protocol was IP/TCP, but other than those, it would be FTP.
-IP Version 4 (99.54%)
      -TCP (96.50%)
            -SSL (7.39%)
            -HTTP (8.78%)
            ...
            -XMPP (0.01%)
      -UDP (1.46%)
      -Generic Routing Encapsulation (1.28%)
      -ICMP (0.29%)
-IP Version 6 (0.35%)
       -UDP (0.17%)
       -ICMP (0.18%)
...
20. A fun question: what other interesting things did you find in this PCAP set (e.g., files)?
	-Multiple papers from the SANS institute InfoSec Reading Room on biometric authentication of users for e-commerce, insecurities in smart cards, upgrading SAP security, two-factor authentication, S/Key usage with OpenBSD, different authentication models, SSO and business reality, and a whole host of more security related articles and papers. The irony of sending security-related (and copyrighted) papers over an unsecured network is amazing. These are all from following TCP Streams and saving the raw data from the FTP-DATA.
	-Many pictures and .js or .css files from using the "Export Objects -> HTTP" command from the Wireshark File menu. In there was a flyer about a 3D printer lecture where it looked like you make one, which I thought was pretty interesting. These ones are not included with the submission, but the security PDFs are included.
